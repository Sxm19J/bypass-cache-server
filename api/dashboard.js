  ...
  res.status(200).send(`
    ...
    <head>
      ...
      <style>
        ...
        .timestamp { color: #64748b; font-size: 0.8rem; margin-left: 10px; }
        .url { color: #e2e8f0; }
      </style>
    </head>
    <body>
      <div class="container">
        ...
        <div class="recent-list">
          <h3 style="margin-top: 0; color: #e2e8f0;">Recent Entries</h3>
          <div id="recent"></div>
        </div>
      </div>
            ...
            if (data.recent && data.recent.length > 0) {
              recentDiv.innerHTML = data.recent.map(item => \`
                <div class="recent-item">
                  <span class="url">\${item.original?.substring(0, 50) || 'Unknown'}...</span>
                  <span class="timestamp">\${new Date(item.timestamp).toLocaleString()}</span>
                </div>
              \`).join('');
            } else {
              recentDiv.innerHTML = '<div class="recent-item">No recent entries</div>';
            }
          } catch (error) {
